...
import cartReducer, { addToCart, clearCart, removeFromCart, updateCartItemQuantity } from "../redux/slices/cartSlice";
...
describe('addToCart', () => {
  ...
  test('should add item to cart if it does not exist', () => {
    ...
    const nextState = cartReducer(initialState, addToCart(action.payload as CartItem));
  ...
  test('should remove item from cart if it exists', () => {
    ...
    const stateAfterAddition = cartReducer(initialState, addToCart(addToCartAction.payload as CartItem));
  ...
  test('should clear all items from the cart', () => {
    ...
    const stateAfterAddition = cartReducer(initialState, addToCart(addToCartAction1.payload as CartItem));
